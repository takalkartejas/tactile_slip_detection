...
class ml_algorithm():
    ...
    def calculate_center_of_mass(self,depth_image):
    ...
    def find_hog_features_using_paths(self, image_paths):
        ...
        for image_path in image_paths:
            ...
            array_max = np.max(hog_features)
            hog_features_normalized = hog_features/array_max
            hog_features_np = np.array(hog_features_normalized)
            x,y = self.calculate_center_of_mass(gray_img)
            weight_factor = 10
            x_normalized = x/640 * weight_factor
            y_normalized = y/480* weight_factor
            hog_features_normalized = np.append(hog_features_normalized,x_normalized)
            hog_features_normalized = np.append(hog_features_normalized,y_normalized)
            # print('hog_features_normalized= ', hog_features_normalized.shape)
            # features.append(x_normalized)
            # features.append(y_normalized)
            features.append(hog_features_normalized)
...
manage_data.process_features(train_data_dir, 28589, train_features_dir)
manage_data.process_features(test_data_dir, 2345, test_features_dir)