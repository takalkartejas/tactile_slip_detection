...
class Manage_data():
    ...
    def create_dataset(self,file_paths, labels):
        ...
        def wrapped_parse_function(filenames, label):
            images, label = tf.py_function(func=self.parse_function_vgg, inp=[filenames, label], Tout=[tf.float32, tf.int32])
...
class AccuracyHistory(Callback):
    ...
    def save_to_csv(self, accuracy_df):
            ...
            file_number = 0
            while True:
                filename = 'D:/rag-tt/tune_log/'+'summary' + str(file_number) + '.csv'
                # Check if the file already exists
                if not os.path.isfile(filename):
                    break
                file_number += 1
            filename_model ='D:/rag-tt/tune_log/'+ 'model' + str(file_number) + '.h5'
