...
class create_network():
    ...
    def train(self, train_dataset, val_dataset):
        ...
        cp = ModelCheckpoint(filepath='checkpoint3.h5',monitor='val_accuracy',save_best_only=True,save_weights_only=False,verbose=1)
        ...
        # Define all custom metrics used in the original model
        custom_objects = {
        ...
        self.model = load_model('checkpoint2.h5',custom_objects)
        # # Evaluate on the validation or test dataset
        # accuracy = self.model.evaluate(val_dataset)
        # print(f"Restored model, accuracy: {accuracy}")
        #tensor board
...
class AccuracyHistory(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # accuracy_df = self.create_accuracy_dataframe()

        # # Save the DataFrame to a CSV file
        # self.save_to_csv(accuracy_df)
