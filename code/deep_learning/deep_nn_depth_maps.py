...
class Manage_data():
    ...
    def trim_data(self, label, image_paths,depth_image_paths, csv_path):
        ...
        # Remove the same number of elements from the start of image_paths and depth_image_paths
        paths_with_few_zeroes = image_paths[zeroes_to_remove:]
        depth_paths_with_few_zeroes = depth_image_paths[zeroes_to_remove:]
        ...
        for slip_value in slip_values:
            if slip_value < tune.max_labels:
                if max_tranistion_trim_value < slip_value or slip_value < min_tranistion_trim_value:
                    ...
                    trimmed_depth_paths.append(depth_paths_with_few_zeroes[i])
            i += 1
        trimmed_labels = np.array(trimmed_labels)
        trimmed_paths = np.array(trimmed_paths)
        trimmed_depth_paths = np.array(trimmed_depth_paths)
        # print(slip_values)
        ...
        return trimmed_labels, trimmed_paths, trimmed_depth_paths
    ...
    def balance_n_separate_data(self, labels, image_paths, depth_image_paths):
        # Convert labels to numpy array for easier manipulation
        labels = np.array(labels)
        image_paths = np.array(image_paths)
        depth_image_paths = np.array(depth_image_paths)
        ...
        if len(class_0_indices) == 0 or len(class_1_indices) == 0:
            print(f"Skipping balancing for {labels} as one of the classes is missing")
            return labels_0,labels_1,image_paths_0,image_paths_1,depth_image_paths_0,depth_image_paths_1
        ...
        for i in class_0_indices:
            ...
            depth_image_paths_0.append(depth_image_paths[i])
        for i in class_1_indices:
            labels_1.append(labels[i])
            image_paths_1.append(image_paths[i])   
            depth_image_paths_1.append(depth_image_paths[i])
        ...
        if diff < 0:  # More 1s than 0s
            ...
            depth_image_paths_0 = np.concatenate([depth_image_paths_0, depth_image_paths[additional_indices]])
        
        # print('labels_0',labels_0)
        # print('image_paths_0',image_paths_0)
        # print('image_paths_0',depth_image_paths_0)
        # print('labels_1',labels_1)
        # print('image_paths_1',image_paths_1)
        # print('image_paths_1',depth_image_paths_1)
        
        return labels_0, image_paths_0, depth_image_paths_0, labels_1, image_paths_1, depth_image_paths_1
    ...
    def load_data(self,data_dir, no_of_samples):
        ...
        for obj_id in range(no_of_samples):
            ...
            for img_id in range(no_of_images):
                ...
                depth_image_paths.append(depth_image_path)
                
            self.check_pattern(label)
            label, image_paths, depth_image_paths = self.trim_data(label,image_paths,depth_image_paths,csv_path)  
            ...
            for i in range(0, len(image_paths_0) - (tune.img_sequence_window_size-1), tune.stride):  # Ensuring sequences of 5 images
                ...
                depth_row = depth_image_paths_0[i:i+tune.img_sequence_window_size]
                image_row = np.array(image_row)
                depth_row = np.array(depth_row)
                row = np.concatenate((image_row, depth_row))
            ...
            for i in range(0, len(image_paths_1) - (tune.img_sequence_window_size-1), tune.stride):  # Ensuring sequences of 5 images
                image_row = image_paths_1[i:i+tune.img_sequence_window_size]
                depth_row = depth_image_paths_1[i:i+tune.img_sequence_window_size]
                image_row = np.array(image_row)
                depth_row = np.array(depth_row)
                row = np.concatenate((image_row, depth_row))
                clubbed_image_paths.append(row)
    ...
    def create_dataset(self, labels, file_paths):
        ...
        def wrapped_parse_function(filenames, label):
            ...
            images.set_shape((tune.img_sequence_window_size*2, 224, 224, 3))  # Explicitly set the shape
...
def test_function():
    ...
    # test_dataset = manage_data.create_dataset(test_labels, test_file_paths) 
    # Print 5 samples
    for idx, (images, label) in enumerate(train_dataset.take(5)):
        print(f"Sample {idx+1}:")
        print("Image shape:", images.numpy().shape)
        print("Label:", label.numpy())
        # print("Image data:", images[0])  # Optional: Print image data if needed
